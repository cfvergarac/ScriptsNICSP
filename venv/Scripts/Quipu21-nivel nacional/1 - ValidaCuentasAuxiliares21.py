...
wb = openpyxl.load_workbook('hoja_bys_=07-02-2018.xlsx')
sheet = wb['ByS']
...
def recorrer():
    for row in range(2, sheet.max_row + 1):
     if( requerimiento[sheet['F' + str(row)].value] <> ['1005', '1011']  ):
      fila = {
            # "ajuste" :  str(sheet['B' + str(row) ].value),
      ...
      #print "* " + fila["grupo"] + fila["subgrupo"] + " - " + fila["requerimiento"] + " E:" + fila["estado"] + " C:" + str(fila["categoria"]) + " P:" + fila["procedencia"]
      validaAux(fila)
      ...
      fila = {
             ...
             "grupo": str(sheet['O' + str(row)].value)[0:3],
             "subgrupo": str((sheet['E' + str(row)].value).split("-")[0]),
             "requerimiento": str(requerimiento[sheet['F' + str(row)].value][0]),
             "procedencia": str((sheet['G' + str(row)].value).split("-")[0]),
             "categoria": str((sheet['I' + str(row)].value).split("-")[0]),
             "celda": str(ajustes[str(sheet['A' + str(row)].value)]),
             "estado": str((sheet['H' + str(row)].value).split("-")[0]),
      ...
      fila = {
         ...
         "grupo": str(sheet['O' + str(row)].value)[0:3],
         "subgrupo": str((sheet['E' + str(row)].value).split("-")[0]),
         "requerimiento": str(requerimiento[sheet['F' + str(row)].value][1]),
         "procedencia": str((sheet['G' + str(row)].value).split("-")[0]),
         "categoria": str((sheet['I' + str(row)].value).split("-")[0]),
         "celda": str(ajustes[str(sheet['A' + str(row)].value)]),
         "estado": str((sheet['H' + str(row)].value).split("-")[0]),
